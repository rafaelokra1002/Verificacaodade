    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={criarFilho} className="card space-y-4">
          <h3 className="text-sm font-bold text-hacker-glow font-mono">
            <span className="text-hacker-dim">$</span> add --target
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} required
                className="input-field text-sm" placeholder="Nome do alvo" />
            </div>
            <div>
              <label className="block text-xs text-hacker-dim mb-1 font-mono">
      ...
      <div className="flex gap-3">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
          className="input-field flex-1 text-sm" placeholder="$ search --target ..." />
      </div>
      ...
      {linkData && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setLinkData(null)}>
          <div className="bg-hacker-card border border-hacker-glow/30 rounded p-6 max-w-lg w-full shadow-neon-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={copiarLink} className="btn-primary flex-1 text-xs font-mono">$ copy_link</button>
              <button onClick={() => setLinkData(null)} className="btn-secondary text-xs font-mono">$ close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {filhos.length === 0 ? (
        <div className="card text-center py-12">
          <div className="text-hacker-dim font-mono text-sm mb-3">
            <p>$ ls ~/targets</p>
            <p className="text-hacker-muted mt-2">// nenhum alvo encontrado</p>
          </div>
          ...
          {filhos.map((filho) => (
            <div key={filho.id} className="card hover:border-hacker-glow/30 transition-all group">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                {[
                  { label: 'pings', value: filho._count.checkins, color: 'text-cyan-400 border-cyan-800/30' },
                  { label: 'fences', value: filho._count.cercas, color: 'text-yellow-400 border-yellow-800/30' },
                  { label: 'alerts', value: filho._count.alertas, color: 'text-red-400 border-red-800/30' },
                ].map((stat) => (
              ...
              {filho.checkins[0] && (
                <div className="text-[10px] text-hacker-dim mb-3 p-2 bg-hacker-bg rounded border border-hacker-border font-mono">
                  {'>'} last_ping: {filho.checkins[0].endereco || 'sem endereço'} — {formatDate(filho.checkins[0].createdAt)}
                </div>
              )}
